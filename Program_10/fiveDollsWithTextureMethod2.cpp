...
void loadTextures() {
	...
	textureImage = SOIL_load_OGL_texture(
		"tshirt.jpg",  // Replace with the path to your texture file
		SOIL_LOAD_AUTO,
		SOIL_CREATE_NEW_ID,
...
// For texture - method 02
unsigned char* image;
GLuint tex;

void loadTexture() {
	image = SOIL_load_image("tshirt.jpg", &width, &height, 0, SOIL_LOAD_RGB); //loads an image into memory without creating an OpenGL texture object.
	if (image == NULL) {
...
void drawFiveDollsWithForLoop() {
	// Doll positions relative to the center doll
	float positions[5][3] = {
		{0.0, 0.0, 0.0},     // center doll
		{2.5, 2.5, 0.0},     // right upper doll
		{2.5, -2.5, 0.0},    // right lower doll
		{-2.5, 2.5, 0.0},    // left upper doll
		{-2.5, -2.5, 0.0}    // left lower doll
	};

	// Loop through all the positions and draw the dolls
	for (int i = 0; i < 5; i++) {
		glPushMatrix();
